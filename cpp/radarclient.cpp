...
Navtech::RadarClient::RadarClient(const std::string& radarAddress, const uint16_t& port)
: _radarClient(std::make_shared<TcpRadarClient>(radarAddress, port)),
	_running(false),
	_sendRadarData(false)
{ }

void Navtech::RadarClient::SetFFTDataCallback(std::function<void(const FFTDataPtr_t&)> callback)
{
...
void Navtech::RadarClient::SetConfigurationDataCallback(std::function<void(const ConfigurationDataPtr_t&)> callback)
{
	std::lock_guard<std::mutex> lock(_callbackMutex);
...
void Navtech::RadarClient::Start()
{
	...
	_radarClient->SetReceiveDataCallback(std::bind(&RadarClient::HandleData, this, std::placeholders::_1));
	_radarClient->Start();
	_running = true;
...
{
	if (_radarClient == nullptr || _radarClient->GetConnectionState() != ConnectionState::Connected)
		return;
	...
	header.SetPayloadLength(sizeof(threshold));
	auto message = std::make_shared<NetworkDataMessage>(header, buffer);	
	
	Helpers::Log("SetNavigationThreshold - Message Length [" + std::to_string(message->MessageData().size()) + "]");
	_radarClient->Send(message->MessageData());
}

void Navtech::RadarClient::SetNavigationGainAndOffset(const float& gain, const float& offset)
{
	if (_radarClient == nullptr || _radarClient->GetConnectionState() != ConnectionState::Connected)
		return;

	std::vector<uint8_t> buffer(sizeof(uint32_t) * 2);
	uint32_t netGain = htonl(static_cast<uint32_t>(gain * RANGEMULTIPLIER));
	uint32_t netOffset = htonl(static_cast<uint32_t>(offset * RANGEMULTIPLIER));
	...
	header.SetPayloadLength(sizeof(uint32_t) + sizeof(uint32_t));
	auto message = std::make_shared<NetworkDataMessage>(header, buffer);	
	_radarClient->Send(message->MessageData());
}

void Navtech::RadarClient::SendSimpleNetworkMessage(const Navtech::CNDPNetworkDataMessageType& type)
{
	if (_radarClient == nullptr || _radarClient->GetConnectionState() != ConnectionState::Connected)
		return;
...
void Navtech::RadarClient::UpdateContourMap(const std::vector<uint8_t>& contourData)
{
	if (_radarClient == nullptr || _radarClient->GetConnectionState() != ConnectionState::Connected)
		return;
	...
	header.SetPayloadLength(static_cast<uint32_t>(contour.size()));
...
void Navtech::RadarClient::HandleData(const CNDPDataMessagePtr_t& message)
{
	...
	case CNDPNetworkDataMessageType::NavigationAlarmData:
		break;
	default:
		Helpers::Log("RadarClient - Unhandled Message [" + std::to_string(static_cast<uint32_t>(message->MessageId())) + "]");
		break;
	}
}

void Navtech::RadarClient::HandleConfigurationMessage(const Navtech::CNDPDataMessagePtr_t& configMessage)
{
	...
	std::memcpy(&configHeader, configMessage->MessageData().data(), configHeader.HeaderLength() + configHeader.header.HeaderLength());
	...
	_expectedRotationRate = ntohs(configHeader.rotationspeed) / 1000;
	...
	if(configurationDataCallback == nullptr) return;
	
	configurationDataCallback(configurationData);
}

void Navtech::RadarClient::HandleFFTDataMessage(const Navtech::CNDPDataMessagePtr_t& fftDataMessage)
{	
	_callbackMutex.lock();
...
void Navtech::RadarClient::HandleNavigationDataMessage(const Navtech::CNDPDataMessagePtr_t& navigationMessage)
{
	_callbackMutex.lock();
	auto navigationDataCallback = _navigationDataCallback;
	_callbackMutex.unlock();	
	if(navigationDataCallback == nullptr) return;
	
	uint16_t netAzimuth = 0;
	std::memcpy(&netAzimuth, &navigationMessage->Payload()[0], sizeof(netAzimuth));
	uint32_t netSeconds = 0;
	std::memcpy(&netSeconds, &navigationMessage->Payload()[sizeof(netAzimuth)], sizeof(netSeconds));
	uint32_t netSplitSeconds = 0;
	std::memcpy(&netSplitSeconds, &navigationMessage->Payload()[sizeof(netAzimuth) + sizeof(netSeconds)], sizeof(netSplitSeconds));
	...
	navigationData->Angle = (navigationData->Azimuth * 360.0f) / (float)_encoderSize;
		
	auto offset = sizeof(netAzimuth) + sizeof(netSeconds) + sizeof(netSplitSeconds);
	auto peaksCount = (navigationMessage->PayloadSize() - offset) / NAVDATARECORDLENGTH;
	for(auto i = offset; i < (10 + (peaksCount * NAVDATARECORDLENGTH)); i += NAVDATARECORDLENGTH)
	{
		uint32_t peakResolve = 0;
		std::memcpy(&peakResolve, &navigationMessage->Payload()[i], sizeof(peakResolve));
		uint16_t power = 0;
		std::memcpy(&power, &navigationMessage->Payload()[i + sizeof(peakResolve)], sizeof(power));
		navigationData->Peaks.push_back(std::make_tuple<float, uint16_t>(htonl(peakResolve) / RANGEMULTIPLIERFLOAT, htons(power)));
	}
