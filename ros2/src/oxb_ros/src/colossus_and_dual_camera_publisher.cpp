...
Colossus_and_dual_camera_publisher::Colossus_and_dual_camera_publisher():Node{ "colossus_and_dual_camera_publisher" }
{
    ...
    declare_parameter("camera_right_url", "");

    radar_ip = get_parameter("radar_ip").as_string();
    radar_port = get_parameter("radar_port").as_int();
    camera_left_url = get_parameter("camera_left_url").as_string();
...
void Colossus_and_dual_camera_publisher::fft_data_handler(const Fft_data::Pointer& data)
{
	...
	if (camera_left_message.image_rows <= 0 || camera_left_message.image_cols <= 0 || camera_left_message.image_channels <= 0 || camera_left_message.image_fps <= 0 ||
        camera_right_message.image_rows <= 0 || camera_right_message.image_cols <= 0 || camera_right_message.image_channels <= 0 || camera_right_message.image_fps <= 0)
	{
        return;
	}
    ...
    if (data->azimuth < last_azimuth) {
        rotated_once = true;
        camera_left_image_publisher->publish(camera_left_message);
        camera_right_image_publisher->publish(camera_right_message);
    }
