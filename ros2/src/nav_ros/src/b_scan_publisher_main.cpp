...
int main(int argc, char* argv[])
{
    ...
    node->get_radar_client()->set_fft_data_callback(std::bind(&B_scan_colossus_publisher::fft_data_handler, node.get(), std::placeholders::_1));
    node->get_radar_client()->set_configuration_data_callback(std::bind(&B_scan_colossus_publisher::configuration_data_handler, node.get(), std::placeholders::_1));
